        ...
        toWriteTextObjects.some((toWriteTextObject, index) => {
            ...
            const writeText = (context, x, y, wto) => {
                ...
                if (options.opacity === 1 && options.colorspace !== 'separation' &&
                    (options.rotation === 0 || options.rotation === undefined)) {
                        console.log('writing directly probably');
                    context.q();
                    ...
                    console.log('indirectly writing??')
                    this.pauseContext();
            ...
            if (!isContinued) { // flush out current line before processing next one
                ...
                if(text.includes(imageKeyword)){
                    ...
                    console.log(this);
                    try{
                        this.image(imgSrc, this.x, this.y + 15, {width: 200, align: 'center'});
                    }
                    catch(error){
            ...
            {
                //console.log(toWriteTextObject, 'asd');

                //console.log("manual vertical");
                //followingBreak = true;
                //numBreaksFollowed++;
...
exports._clayoutText = function _clayoutText(textObjects, textBox, pathOptions) {
    ...
    const writeValue = (textObject) => {
       // //console.log(textObject, "whatistext");
        ...
        if(textObject.value!='[@@DONOT_RENDER_THIS@@]'&&textObject.value!=null)
        {
            let firstWord = textObject.value.split(" ")[0];
            let roomCheckWord = new Word(firstWord, pathOptions);
            let wordSpace = roomCheckWord.dimensions;
            if(wordSpace.width + continuingLineWidth < 206)
            {
                enoughRoom = true;
                //console.log("enoughroom calculated");
            }
        }
        */
