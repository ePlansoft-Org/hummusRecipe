        ...
        toWriteTextObjects.some((toWriteTextObject, index) => {
            ...
            const writeText = (context, x, y, wto) => {
                ...
                if (options.opacity === 1 && options.colorspace !== 'separation' &&
                    ...
                    //console.log("giving this much space", lineHeight);
                    //console.log('text is', text)
                    //console.log("this info is av", textBox)
            ...
            if(text.includes(imageKeyword)){
                ...
                console.log('text object was image', toWriteTextObject)
                console.log('recorded height', lineHeight)
                const hash = imgSrc.substring(imgSrc.indexOf(imageDownloadLink)+imageDownloadLink.length);
                imgSrc = '/tmp/' + hash 
                //console.log(imgSrc);
                try{
                    this.image(imgSrc, this.x, this.y + 30, {width: scaledImageWidth, align: 'center', commentImage: true});
                }
                catch(error){
...
function makeTextObject(lines, line, lineID, textBox, options = {}, hum = {}) {
   ...
   if(line.value.includes(imageKeyword)){
        ...
        let addedHeight = Math.floor(imageWidth / ratio)
