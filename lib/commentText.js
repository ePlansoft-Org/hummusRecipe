...
//when we write comment images, we need to know that the comment has an image
//via just the text it has... so use keyword
var imageKeyword = '1mag3()L1nk';
...
exports.ctext = function ctext(text = '', x, y, options = {}) {
    ...
    console.log('endtextobs');
    for (let i=0; i<textObjects.length; i++){
        for (let x=0; x<textObjects[i].childs.length; x++){
            console.log('childs');
            console.log(textObjects[i].childs[x]);
            console.log('endchilds');
        }
    }
    ...
    */
    ...
    //console.log('twtxtobs');
    //console.log(toWriteTextObjects);
    //console.log('endtwtxtobs');
    if (!textBox.width) {
    ...
    if (this._flow) {
        ...
        toWriteTextObjects.some((toWriteTextObject, index) => {
            ...
            //INSERT IMAGES HERE PROBABLY????
            //console.log(text);
            if(text.includes(imageKeyword)){
                let imgSrc = text.substring(imageKeyword.length);
                //the folder we downloaded the image into
                imgSrc = '/tmp/' + imgSrc.substring(imgSrc.indexOf('Images') + 'Images'.length + 1).split('%20').join(' ');
                //console.log(imgSrc);
                try{
                    //let existing image function handle
                    this.image(imgSrc, this.x, this.y + 15, {width: scaledImageWidth, align: 'center'});
                }
                catch(error){
                    console.log('ERROR PRINTING COMMENT IMAGES', error);
                }
                //remove image link from text so that the link is not printed
                toWriteTextObject.text = ''
            }
...
exports._clayoutText = function _clayoutText(textObjects, textBox, pathOptions) {
    ...
    const writeValue = (textObject) => {
        ...
        if ((textObject.value !== undefined && textObject.value !== null)) {
            ...
            //this line does nothing right now, but the link attribute does exist, so maybe in the future we can pull in changes that let us hyperlink images...
            textObject.link = textObject.attributes.find(element => 'name' in element && element.name=='src').value;
            //append our keyword to the textobject so that, after text processing, we can still know this is an image
...
function makeTextObject(lines, line, lineID, textBox, options = {}, hum = {}) {
    //cannot use keyword "self", so passing in self as hum in order to use getImageDimensions
