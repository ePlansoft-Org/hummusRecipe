 ...
 * @param {Array} [options.responseComments] - Array of annotation replies
 */
exports.annot = function annot(x, y, subtype, options = { text: '', width: 0, height: 0 }) {
    const { text, width, height, responseComments } = options;
    this.annotationsToWrite.push({
        subtype,
...
exports._annot = function _annot(subtype, args = {}, pageNumber, ref) {
    const { x, y, width, height, options, responseComment } = args;
    let { text } = args;
    ...
    if (responseComment && ref) {
        text = responseComment.content;
        params.date = responseComment.date;
        params.title = responseComment.title;
    }
...
exports._writeAnnotations = function _writeAnnotations() {
    this.annotationsToWrite.forEach((annot) => {
        ...
        if (annot.responseComments) {
            annot.responseComments.forEach((response) => {
                annot.args.responseComment = response;
                annot.args.responseComment.date = response.updatedAt;
