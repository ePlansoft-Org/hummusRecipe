 ...
 */
 exports.image = function image(imgSrc, x, y, options = {}) {
    ...
    console.log(y);
    console.log((options));
    console.log('trying to do image');
    //imgSrc+='.jpeg'
...
exports._getImgOffset = function _getImgOffset(imgSrc = '', options = {}) {
    ...
    if (options.width && !options.height) {
        if  (dimensions.width > options.width || !options.commentImage){
            width = options.width;
        }
        height = options.width / ratio;
        height = (options.commentImage && height > options.width) ? options.width : height;
    } else
    if (!options.width && options.height) {
