...
exports.Line = class Line {
    ...
    /** 
    addSpacelessWord(wordObject){
        let temp = this.wordObjects.pop();
        this.wordObjects.push(new Word(temp._value+wordObject._value, temp._pathOptions));
    }
    */
    ...
    markLastWord() {
        //console.log("linethinkswordmarked");
    ...
    canFit(wordObject) {
        //console.log(this.value, "value", wordObject.value);
        const tempValue = this.value + wordObject.value;
        //console.log(this.value, "fsdjghsrteiiogjnrjth", wordObject.value);
        let toWidth = this._pathOptions.font.calculateTextDimensions(
            tempValue, this.size
        ).xMax + this.charSpacing(tempValue);
        toWidth = this._bold ? toWidth*1.03 : (toWidth);
        //console.log(toWidth, this.width, tempValue, "widths");
    ...
    difference(wordObject){
        ...
        const currWidth = this._pathOptions.font.calculateTextDimensions(
            this.value, this.size
        ).xMax + this.charSpacing(this.value);

        diff= toWidth - currWidth;
        return this._bold ? diff*1.03 : (diff);
    }
    ...
    get currentWidth() {
        ...
        return (this._pathOptions.font.calculateTextDimensions(
            this.value, this.size
        ).xMax + this.charSpacing(this.value)) * multiplier * 43/32;
    }
