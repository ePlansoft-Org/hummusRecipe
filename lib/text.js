...
exports.text = function text(text = '', x, y, options = {}) {
    ...
    if (this._flow) {
        ...
        toWriteTextObjects.some((toWriteTextObject, index) => {
            ...
            //console.log(toWriteTextObject, isContinued, "towritetext");
...
exports._layoutText = function _layoutText(textObjects, textBox, pathOptions) {
    ...
    const writeValue = (textObject) => {
        console.log(textObject, "whatistext");
...
function makeTextObjects(self, textObject = {}, pathOptions, textBox = {}) {
    ...
    let isSomething = textObject.isBold || textObject.isItalic || textObject.underline;
   // console.log(isSomething, textObject.isBold, "something?");
    if(!isSomething && !followingAspect)
    {
        console.log("zeroed out :(");
        continuingLineWidth = 0;
        
        followingAspect = false;
    }
    else{
        if(isSomething)
        {
    ...
    let originalText = ((textObject.prependValue) ? textObject.prependValue : '') +
        textObject.value +
        ((textObject.appendValue) ? textObject.appendValue : '');
    let text = originalText;
    let existingWords = ["test"];
    //if(isSomething){
        existingWords = originalText.split(" ");
   // }
    if(existingWords.length>1&&existingWords[existingWords.length-1]=='')
    {
        existingWords= existingWords.slice(0, existingWords.length-1);
        
    }
    for(let i=0; i<existingWords.length-1; i++)
    {
        existingWords[i]= existingWords[i]+" ";
    }
   ...
   //   breaker = new LineBreaker(text);
    ...
    do{
        //if(isSomething)
        //{
            breaker = new LineBreaker(existingWords[counter]);
        //}
    let bk = breaker.nextBreak();
    /** 
    if(bk==null)
    {
        console.log(existingWords, counter, "nullcheck");
    }
    else
    {
        console.log(existingWords, counter, "antinull");
    }
    */
    ...
    while (bk) {
        ...
        let word;
        //if(!isSomething){
         //  word  = nextWord(text, bk, last, pathOptions);
        //}
       // else{
            word = new Word(existingWords[counter], pathOptions);
       // }
    console.log(word._value, continuingLineWidth, "contline");
    //console.log(newLine.wordObjects);
    //console.log(newLine.wordObjects);
    //newLine.width
        if (newLine.canFit(word) && continuingLineWidth + newLine.difference(word) < 206) {
            console.log("testing gets", newLine.value, newLine.currentWidth);
            console.log("could fit");
            killedLine = false;
           ...
           // if(!newLine.canFit(word)){
            //console.log(continuingLineWidth, "continental");
            if(isSomething)
                //continuingLineWidth < newLine.width)
            {
                //const previousLine = toWriteTextObjects[Math.max(toWriteTextObjects.length - 1,0)];
                //console.log(newLine.lineID, previousLine.lineID, textObject.lineID, "IDS HERE");
                //newLine.lineID = previousLine.lineID;
                //textObject.lineID = previousLine.lineID;
                //console.log(newLine.lineID != previousLine.lineID, "comparison test");
            }
            continuingLineWidth = newLine.difference(word);
            killedLine = true;
    ...
    console.log(isLastLine, "was last line", killedLine);
    if (!flushLine) {
        [wordCount, totalTextWidth] =
        bindTextToLine(newLine, toWriteTextObjects, wordCount, totalTextWidth);
